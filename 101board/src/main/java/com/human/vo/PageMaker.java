...
//		private int endPage;//pageMaker에서 마지막 페이지 번호
	    endPage = (int) (//올림 floor 내림 round 반올림
	    		Math.ceil(page /(double) displayPageNum) // 얘는 반올림
	    		* displayPageNum);
//	    private int startPage;//pageMake에서 시작 페이지 번호
	    startPage=endPage-displayPageNum+1;
	    if(totalEndPage < endPage) { //totalEndPage가 endPage 보다 작을경우
	    	endPage=totalEndPage; //endPage를 totalEndPage로 바꿔준다.
	    }
	    
	    if(startPage<1) {
	    	startPage=1; //startPage가 1보다 작으면 안되니까 안전장치
	    }
	    if(startPage==1) {
	    	prev=false; // startPage가 1이면 이전페이지 버튼을 없앰
	    }else {
	    	prev=true;
	    }
	    if(endPage==totalEndPage) {
	    	next=false; // endPage랑 totalEndPage가 같으면 다음버튼 없앰 
	    }else {
	    	next=true; 
	    }
